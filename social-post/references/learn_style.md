...
## 7. Writing Voice / Writing Style 標籤法（補充分析層）

> 來源：`Prompt/如何訓練 ChatGPT，模仿你的語氣來寫文章？.md`（Matt Giaro《The Automated Sales Machine》）。在第 5 步質性分析之外，額外產出兩組標籤化結果，寫入 `style_profile.md` 的「Writing Voice / Writing Style 標籤」段落，當作生成前的快速校準清單。

- **Writing Style**：從樣本中挑出最多 10 個風格形容詞（例：簡單、交談式、樂觀、幽默、權威……），描述全篇一致的調性。
- **Writing Voice**：依 6 個準則描述（不含語氣/聲音本身）：
  1. 用詞選擇：具體詞彙與術語偏好
  2. 句子結構：長度、複雜度
  3. 節奏與速度：流暢度與節奏感
  4. 觀點：敘事視角（第一人稱/集體）
  5. 修辭手法：隱喻、明喻、對立句式等文學手法
  6. 意象：為讀者創造畫面的描述性語言

兩組標籤跟第 5 步的質性描述（語氣/句長/標點…）互補，不重複覆寫，差異是這兩組更適合當「一眼校準」用的速查清單。
