...
| 說「配圖」「生圖」「封面圖」「幫這篇做圖」 | **P2-Image** | `references/generate_image.md` + `image_prompt_profile.md` + `style_profile.md` + 該篇文案 |
...
- **P1.5 圖像也要套用品牌**：若使用者要配圖或封面圖，先讀 `image_prompt_profile.md`，優先套個人視覺語言與公司 CI，不要每次從零亂寫 prompt
...
- **原文保護**：若任務是改寫外部文章、筆記、voicenote、草稿，**不要直接覆寫原文**。改寫版本一律另存到 `/Users/fishtv/Development/book/social-post/`，保留原始檔案不動。
